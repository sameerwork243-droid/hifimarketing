...
// ----------------------------------------------------------------------
// FIX #1: Whitelist the tab value. Previously $_GET['tab'] was used
// directly with no validation, and the navbar links pointed to files
// like deliverables.php / tickets.php / verbal.php / progress-sync.php
// that live OUTSIDE this file's own tab-routing logic. If those files
// don't exist (or their own session/role bootstrap fails) PHP/the
// browser falls through to the header('Location: client-portal.php')
// redirect above using a RELATIVE path -- which is exactly the
// "redirects to client-portal/client-portal.php" bug being reported.
//
// Since this file already contains full working markup for every tab
// (operations, deliverables, tickets, verbal, sync), the safest fix is
// to route ALL of those tabs through THIS file via ?tab=xxx, instead of
// linking out to separate physical files that don't share this logic.
// ----------------------------------------------------------------------
...
<header>
    <div class="header-inner">
        ...
        <!--
            FIX: nav links now point back to THIS file (pm-portal.php)
            using ?tab=xxx instead of separate files (index.php,
            deliverables.php, tickets.php, verbal.php, progress-sync.php).
            Those separate files were causing the client-portal.php
            redirect bug. pm-billing.php stays a separate physical file
            since it is not one of this file's internal tabs -- make sure
            it uses the SAME session/role check block as the top of this
            file, with an ABSOLUTE (not relative) redirect path.
        -->
        <nav class="desktop-nav">
            <a href="pm-portal.php?tab=operations" class="nav-link <?php echo $activeTab === 'operations' ? 'active' : ''; ?>">
                <i class="fas fa-tachometer-alt"></i> Operations
            </a>
            <a href="pm-portal.php?tab=deliverables" class="nav-link <?php echo $activeTab === 'deliverables' ? 'active' : ''; ?>">
                <i class="fas fa-check-square"></i> Deliverables
            </a>
            <a href="pm-portal.php?tab=tickets" class="nav-link <?php echo $activeTab === 'tickets' ? 'active' : ''; ?>">
                <i class="fas fa-headset"></i> Tickets
            </a>
            <a href="pm-portal.php?tab=verbal" class="nav-link <?php echo $activeTab === 'verbal' ? 'active' : ''; ?>">
                <i class="fas fa-phone"></i> Verbal
            </a>
            <a href="pm-portal.php?tab=sync" class="nav-link <?php echo $activeTab === 'sync' ? 'active' : ''; ?>">
    ...
    <nav class="mobile-nav" id="mobileNav">
        ...
        <div class="mobile-links">
            <a href="pm-portal.php?tab=operations" class="<?php echo $activeTab === 'operations' ? 'active' : ''; ?>" onclick="closeMobileMenu()">
                <i class="fas fa-tachometer-alt"></i> Operations Desk
            </a>
            <a href="pm-portal.php?tab=deliverables" class="<?php echo $activeTab === 'deliverables' ? 'active' : ''; ?>" onclick="closeMobileMenu()">
                <i class="fas fa-check-square"></i> Manage Deliverables
            </a>
            <a href="pm-portal.php?tab=tickets" class="<?php echo $activeTab === 'tickets' ? 'active' : ''; ?>" onclick="closeMobileMenu()">
                <i class="fas fa-headset"></i> Client Tickets & Tasks
            </a>
            <a href="pm-portal.php?tab=verbal" class="<?php echo $activeTab === 'verbal' ? 'active' : ''; ?>" onclick="closeMobileMenu()">
                <i class="fas fa-phone"></i> Client Verbal Requests
            </a>
            <a href="pm-portal.php?tab=sync" class="<?php echo $activeTab === 'sync' ? 'active' : ''; ?>" onclick="closeMobileMenu()">
        ...
        <div class="content">
            ...
            <div class="card">
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <select id="attachment-client-select" onchange="window.location.href='pm-portal.php?tab=<?php echo htmlspecialchars($activeTab); ?>&client_id=' + this.value" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:8px;background:#f8fafc;font-size:13px;">
