<?php
// pm-billing.php - PM Invoice Management (FINAL WORKING)
...
if (isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'download_attachment') {
        ...
        if ($invoice_id > 0) {
            ...
            if ($row && !empty($row['attachment'])) {
                $response = ['success' => true, 'file' => $row['attachment']];
            } else {
                $response = ['success' => false, 'message' => 'No attachment found'];
            }
        } else {
...
<head>
    ...
    <title>PM Billing Management</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .desktop-nav { display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
        .desktop-nav .nav-link { font-weight: 600; font-size: 12px; color: var(--text-secondary); padding: 6px 12px; border-radius: 8px; transition: var(--transition); }
        ...
        .client-selector { background: var(--card-bg); border-radius: var(--radius); border: 1px solid var(--border); padding: 16px 20px; margin-bottom: 16px; display: flex; align-items: center; gap: 16px; flex-wrap: wrap; box-shadow: var(--shadow); }
        .client-selector label { font-weight: 600; font-size: 13px; color: var(--text-secondary); }
        .client-selector select { padding: 8px 16px; border: 1px solid var(--border); border-radius: 8px; font-size: 13px; font-family: 'Inter', sans-serif; background: #f8fafc; min-width: 200px; cursor: pointer; }
        .client-selector select:focus { outline: none; border-color: var(--primary); }
        .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 20px; }
        .stat-card { background: var(--card-bg); border-radius: var(--radius); border: 1px solid var(--border); padding: 14px 16px; transition: var(--transition); }
        .stat-card:hover { box-shadow: var(--shadow-hover); }
        .stat-card .number { font-size: 20px; font-weight: 900; color: var(--text-primary); }
        .stat-card .label { font-size: 11px; color: var(--text-muted); }
        .stat-card .stat-icon { float: right; font-size: 20px; opacity: 0.15; color: var(--primary); }
        .table-wrap { overflow-x: auto; }
        table { width: 100%; border-collapse: collapse; font-size: 12px; }
        table th { background: #f8fafc; text-align: left; padding: 10px 12px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); border-bottom: 2px solid var(--border); }
        table td { padding: 10px 12px; color: var(--text-secondary); border-bottom: 1px solid #f0f2f5; vertical-align: middle; }
        table tr:hover td { background: #f8fafc; }
        .status-badge { padding: 3px 10px; border-radius: 20px; font-size: 9px; font-weight: 700; display: inline-block; text-transform: uppercase; }
        .status-badge.paid { background: #d1fae5; color: #065f46; }
        .status-badge.due { background: #fee2e2; color: #dc2626; }
        .status-badge.partially-paid { background: #fef3c7; color: #92400e; }
        .btn-sm { padding: 4px 10px; border-radius: 6px; border: none; font-size: 10px; font-weight: 600; cursor: pointer; transition: var(--transition); }
        ...
        .btn-sm.status { background: #e8edfe; color: var(--primary); }
        .btn-sm.status:hover { background: var(--primary); color: #fff; }
        ...
        @media (max-width: 1024px) { .stats-grid { grid-template-columns: repeat(2, 1fr); } .modal .form-grid { grid-template-columns: 1fr; } }
        ...
        @media (max-width: 768px) { .sidebar { display: none; } .main-layout { padding: 12px; flex-direction: column; } .banner { padding: 16px 18px; flex-direction: column; text-align: center; } .banner h2 { font-size: 16px; } .stats-grid { grid-template-columns: 1fr 1fr; } .client-selector { flex-direction: column; align-items: stretch; } .client-selector select { width: 100%; } .header-actions .action-btn { padding: 4px 8px; font-size: 13px; } .modal { padding: 20px; } .header-inner { padding: 0 12px; } .logo { font-size: 17px; } .logo .brand-icon { width: 30px; height: 30px; font-size: 13px; } .banner .banner-actions .btn-white { width: 100%; text-align: center; } table { font-size: 11px; } table th, table td { padding: 6px 8px; } }
        @media (max-width: 480px) { .stats-grid { grid-template-columns: 1fr; } .header-actions .action-btn { font-size: 12px; padding: 4px 6px; } .header-actions .user-badge { padding: 2px 8px 2px 2px; font-size: 11px; } .header-actions .user-badge img { width: 24px; height: 24px; } .mobile-nav { width: 280px; } }
...
<body>
    ...
    <header>
        <div class="header-inner">
            ...
            <nav class="desktop-nav">
                ...
                <a href="pm-billing.php" class="nav-link active"><i class="fas fa-file-invoice"></i> Billing</a>
            </nav>
    ...
    <nav class="mobile-nav" id="mobileNav">
        ...
        <div class="mobile-links">
            <a href="index.php" onclick="closeMobileMenu()"><i class="fas fa-tachometer-alt"></i> Operations Desk</a>
            ...
            <a href="pm-billing.php" class="active" onclick="closeMobileMenu()"><i class="fas fa-file-invoice"></i> Billing Management</a>
            <a href="service-packages.php" onclick="closeMobileMenu()"><i class="fas fa-boxes"></i> Service Packages</a>
        </div>
    ...
    <div class="main-layout">
        ...
        <aside class="sidebar <?php echo $isCollapsed ? 'collapsed' : ''; ?>" id="mainSidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="service-packages.php" class="sidebar-link"><i class="fas fa-bullhorn"></i><span class="sidebar-text">Ad Compaigns</span></a>
                <a href="service-packages.php" class="sidebar-link"><i class="fas fa-boxes"></i><span class="sidebar-text">Service Packages</span></a>
                <a href="pm-billing.php" class="sidebar-link active"><i class="fas fa-credit-card"></i><span class="sidebar-text">Billing Management</span></a>
        ...
        <div class="content">
            ...
            <div class="banner">
                <div>
                    <h2><i class="fas fa-file-invoice"></i> Billing Management</h2>
                    <p>Manage client invoices &bull; <strong><?php echo $package_name; ?></strong></p>
                </div>
            ...
            <div class="client-selector">
                ...
                <?php endif; ?>
                <button onclick="openModal('modal-add-invoice')" style="margin-left:auto;padding:8px 20px;background:var(--primary);color:#fff;border:none;border-radius:40px;font-weight:700;font-size:12px;cursor:pointer;transition:var(--transition);">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <span style="font-size:12px;font-weight:700;color:var(--text-muted);">
                        <i class="fas fa-info-circle"></i> Manage invoices
                    </span>
                </div>
    ...
    <div class="modal-overlay" id="modal-add-invoice">
        <div class="modal">
            ...
            <h3><i class="fas fa-plus-circle"></i> Add Invoice</h3>
    ...
    <div class="modal-overlay" id="modal-upload">
        <div class="modal">
            ...
            <h3><i class="fas fa-upload"></i> Upload Attachment</h3>
...
function downloadAttachment(invoiceId) {
    ...
    .then(data => {
        ...
        if (data.success && data.file) {
            // Use direct download from admin-portal
            const downloadUrl = 'direct_download.php?file=' + encodeURIComponent(data.file);
