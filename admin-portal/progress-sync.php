<?php
// progress-sync.php - FINAL WORKING VERSION WITH HISTORICAL TRACKING
session_start();
...
// ===== CREATE HISTORICAL TABLE IF NOT EXISTS =====
$create_table = "CREATE TABLE IF NOT EXISTS client_progress_history (
    id INT AUTO_INCREMENT PRIMARY KEY,
    client_id INT NOT NULL,
    posts_completed INT DEFAULT 0,
    stories_completed INT DEFAULT 0,
    reels_completed INT DEFAULT 0,
...
if ($selected_client_id > 0) {
    ...
    // Get historical progress based on filter
    if ($filter_type !== 'all') {
        $date_condition = "";
        if ($filter_type === 'week') {
            $date_condition = "DATE(snapshot_date) >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK)";
        } elseif ($filter_type === 'month') {
            $date_condition = "DATE(snapshot_date) >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)";
        } elseif ($filter_type === 'year') {
            $date_condition = "DATE(snapshot_date) >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)";
        }
        
        $hist_query = "SELECT * FROM client_progress_history 
                       WHERE client_id = $selected_client_id 
                       AND $date_condition 
                       ORDER BY snapshot_date ASC";
        $hist_result = mysqli_query($conn, $hist_query);
        $historical_data = mysqli_fetch_all($hist_result, MYSQLI_ASSOC);
    }
}
...
$current_page = 'progress-sync.php';
    ...
    if ($_POST['ajax_action'] === 'update_social_progress') {
        ...
        if ($client_id > 0) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                // Save to history
                $hist_sql = "INSERT INTO client_progress_history 
                            (client_id, posts_completed, stories_completed, reels_completed, 
                             ads_completed, total_likes, followers_gained, snapshot_date) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, CURDATE())";
                $hist_stmt = mysqli_prepare($conn, $hist_sql);
                mysqli_stmt_bind_param($hist_stmt, "iiiiiii", $client_id, $posts, $stories, $reels, $ads, $likes, $followers);
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filters-section .btn-reset:hover {
            background: #e5e7eb;
        }
        ...
        .progress-card .client-header .filter-badge {
            font-size: 11px;
        ...
        .progress-summary .stat-item .stat-value {
            font-size: 20px;
            font-weight: 800;
            color: var(--primary);
        }
        .progress-summary .stat-item .stat-label {
            font-size: 11px;
            color: var(--text-muted);
        }
        .progress-summary .stat-item .stat-change {
            font-size: 11px;
            font-weight: 600;
        }
        .progress-summary .stat-item .stat-change.positive {
            color: var(--success);
        }
        .progress-summary .stat-item .stat-change.negative {
            color: var(--danger);
        }
        ...
        .service-item .service-left .icon.historical { color: var(--warning); }
        ...
        .service-item .service-right .progress-text .historical-value {
            color: var(--warning);
        }
        ...
        .service-item .service-right input[type="range"]:disabled {
            opacity: 0.5;
        }
        ...
        @media (max-width: 768px) {
            ...
            .filters-section select { min-width: 100%; }
...
<body>
    ...
    <header>
        <div class="header-inner">
            ...
            <div class="header-actions">
                <a href="index.php" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Admin Portal</a>
        ...
        <div class="content">
            ...
            <div class="banner">
                <div>
                    ...
                    <p>Track and update client social media progress with historical data</p>
                </div>
            ...
            <div class="filters-section">
                <label for="filterType"><i class="fas fa-calendar-alt"></i> View:</label>
                <select id="filterType" onchange="applyFilters()">
                    <option value="all" <?php echo $filter_type === 'all' ? 'selected' : ''; ?>>All Time</option>
                    <option value="week" <?php echo $filter_type === 'week' ? 'selected' : ''; ?>>Last Week</option>
                    <option value="month" <?php echo $filter_type === 'month' ? 'selected' : ''; ?>>Last Month</option>
                    <option value="year" <?php echo $filter_type === 'year' ? 'selected' : ''; ?>>Last Year</option>
                </select>
                <button class="btn-filter" onclick="applyFilters()"><i class="fas fa-filter"></i> Apply Filter</button>
                <a href="progress-sync.php<?php echo $selected_client_id > 0 ? '?client_id=' . $selected_client_id : ''; ?>" class="btn-reset"><i class="fas fa-undo"></i> Reset</a>
            </div>

            <!-- ===== CLIENT SELECTOR ===== -->
            <div class="client-selector">
                <label for="sync-client-selector"><i class="fas fa-user"></i> Select Client:</label>
                <select id="sync-client-selector" onchange="window.location.href='?client_id=' + this.value + '&filter=<?php echo $filter_type; ?>'">
            ...
            <?php if ($selected_client && $selected_package): 
                ...
                $latest_historical = null;
                if ($historical_data && count($historical_data) > 0) {
                    $latest_historical = end($historical_data);
                }
            ?>
            ...
            <div class="progress-card">
                <div class="client-header">
                    ...
                    <?php if ($filter_type !== 'all' && $latest_historical): ?>
                        <span class="filter-badge">
                            <?php echo ucfirst($filter_type); ?> View
                        </span>
                    <?php endif; ?>
                </div>
                ...
                <div class="progress-summary">
                    <div class="stat-item">
                        <div class="stat-value"><?php echo $progress['postsCompleted']; ?></div>
                        <div class="stat-label">Posts Done</div>
                        <?php if ($latest_historical): ?>
                            <div class="stat-change <?php echo $progress['postsCompleted'] >= $latest_historical['posts_completed'] ? 'positive' : 'negative'; ?>">
                                <?php 
                                    $diff = $progress['postsCompleted'] - $latest_historical['posts_completed'];
                                    echo ($diff >= 0 ? '+' : '') . $diff . ' from previous';
                                ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="stat-item">
                        <div class="stat-value"><?php echo $progress['storiesCompleted']; ?></div>
                        <div class="stat-label">Stories Done</div>
                        <?php if ($latest_historical): ?>
                            <div class="stat-change <?php echo $progress['storiesCompleted'] >= $latest_historical['stories_completed'] ? 'positive' : 'negative'; ?>">
                                <?php 
                                    $diff = $progress['storiesCompleted'] - $latest_historical['stories_completed'];
                                    echo ($diff >= 0 ? '+' : '') . $diff . ' from previous';
                                ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="stat-item">
                        <div class="stat-value"><?php echo number_format($progress['followersGained']); ?></div>
                        <div class="stat-label">Followers Gained</div>
                        <?php if ($latest_historical): ?>
                            <div class="stat-change <?php echo $progress['followersGained'] >= $latest_historical['followers_gained'] ? 'positive' : 'negative'; ?>">
                                <?php 
                                    $diff = $progress['followersGained'] - $latest_historical['followers_gained'];
                                    echo ($diff >= 0 ? '+' : '') . number_format($diff) . ' from previous';
                                ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="stat-item">
                        <div class="stat-value"><?php echo number_format($progress['totalLikes']); ?></div>
                        <div class="stat-label">Total Likes</div>
                        <?php if ($latest_historical): ?>
                            <div class="stat-change <?php echo $progress['totalLikes'] >= $latest_historical['total_likes'] ? 'positive' : 'negative'; ?>">
                                <?php 
                                    $diff = $progress['totalLikes'] - $latest_historical['total_likes'];
                                    echo ($diff >= 0 ? '+' : '') . number_format($diff) . ' from previous';
                                ?>
                            </div>
                        <?php endif; ?>
                ...
                <div class="service-grid">
                    ...
                    <?php if ($posts_limit > 0): 
                        ...
                        $hist_pct = $latest_historical ? round(($latest_historical['posts_completed'] / $posts_limit) * 100) : 0;
                    ?>
                    <div class="service-item">
                        <div class="service-left">
                            <span class="icon <?php echo $filter_type !== 'all' && $latest_historical ? 'historical' : 'active'; ?>">
                                <i class="fas fa-<?php echo $filter_type !== 'all' && $latest_historical ? 'clock' : 'check-circle'; ?>"></i>
                            </span>
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="posts-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo $latest_historical['posts_completed']; ?></span> → 
                                <?php endif; ?>
                    ...
                    <?php if ($stories_limit > 0): 
                        ...
                        $hist_pct = $latest_historical ? round(($latest_historical['stories_completed'] / $stories_limit) * 100) : 0;
                    ?>
                    <div class="service-item">
                        <div class="service-left">
                            <span class="icon <?php echo $filter_type !== 'all' && $latest_historical ? 'historical' : 'active'; ?>">
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="stories-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo $latest_historical['stories_completed']; ?></span> → 
                                <?php endif; ?>
                    ...
                    <?php if ($reels_limit > 0): 
                        ...
                        $hist_pct = $latest_historical ? round(($latest_historical['reels_completed'] / $reels_limit) * 100) : 0;
                    ?>
                    <div class="service-item">
                        <div class="service-left">
                            <span class="icon <?php echo $filter_type !== 'all' && $latest_historical ? 'historical' : 'active'; ?>">
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="reels-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo $latest_historical['reels_completed']; ?></span> → 
                                <?php endif; ?>
                    ...
                    <?php if ($ads_limit > 0): 
                        ...
                        $hist_ads_pct = $latest_historical ? round(($latest_historical['ads_completed'] / $ads_limit) * 100) : 0;
                    ?>
                    <div class="service-item">
                        <div class="service-left">
                            <span class="icon <?php echo $filter_type !== 'all' && $latest_historical ? 'historical' : 'active'; ?>">
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="ads-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo $latest_historical['ads_completed']; ?></span> → 
                                <?php endif; ?>
                    ...
                    <div class="service-item">
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="likes-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo number_format($latest_historical['total_likes']); ?></span> → 
                                <?php endif; ?>
                            ...
                            <input type="range" id="sync-likes-<?php echo $selected_client['id']; ?>" 
                                   ...
                                   oninput="document.getElementById('likes-display-<?php echo $selected_client['id']; ?>').textContent = 
                                   <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                       '<?php echo number_format($latest_historical['total_likes']); ?> → ' + 
                                   <?php endif; ?>
                    ...
                    <div class="service-item">
                        ...
                        <div class="service-right">
                            <span class="progress-text" id="followers-display-<?php echo $selected_client['id']; ?>">
                                <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                    <span class="historical-value"><?php echo number_format($latest_historical['followers_gained']); ?></span> → 
                                <?php endif; ?>
                            ...
                            <input type="range" id="sync-followers-<?php echo $selected_client['id']; ?>" 
                                   ...
                                   oninput="document.getElementById('followers-display-<?php echo $selected_client['id']; ?>').textContent = 
                                   <?php if ($filter_type !== 'all' && $latest_historical): ?>
                                       '<?php echo number_format($latest_historical['followers_gained']); ?> → ' + 
                                   <?php endif; ?>
                                   Number(this.value).toLocaleString()">
                        </div>
                    </div>
                ...
                <div class="historical-view">
                    <div class="title"><i class="fas fa-clock"></i> Historical Data (<?php echo ucfirst($filter_type); ?>)</div>
                    <p style="font-size:13px;color:#92400e;">
                        Showing progress from <strong><?php echo date('M d, Y', strtotime($latest_historical['snapshot_date'])); ?></strong>
                        vs current progress.
                    </p>
                </div>
                <?php endif; ?>
                ...
                <?php else: ?>
                <button class="save-btn" onclick="saveClientProgress(<?php echo $selected_client['id']; ?>)" disabled>
                    <i class="fas fa-lock"></i> Switch to "All Time" to save progress
                </button>
                <?php endif; ?>
            </div>
            ...
            <div class="sync-footer">
                <i class="fas fa-info-circle"></i> Changes will be synced instantly to the client dashboard. Historical data is automatically saved.
            </div>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            const clientId = document.getElementById('sync-client-selector').value;
            let url = 'progress-sync.php';
            if (clientId > 0) {
                url += '?client_id=' + clientId + '&filter=' + filterType;
            } else {
        ...
        function updateProgressDisplay(clientId, type, value, limit) {
            ...
            if (el) {
                ...
                const hasHistorical = currentText.includes('→');
                if (hasHistorical) {
                    const parts = currentText.split('→');
                    el.textContent = parts[0].trim() + ' → ' + value + '/' + limit + ' (' + pct + '%)';
                } else {
